...
function CourseProgressCard({course}:Props) {
  return (
    ...
    <div className='border-4 rounded-2xl'>
      ...
      width={500}
      height={500}
      className='w-full h-[170px] rounded-2xl rounded-t-xl object-cover '
      />
      <div className='font-game p-4'>
        <h2 className='text-lg font-light text-gray-500'>Course</h2>
        <h2 className='text-2xl'>{course?.title}</h2>
        <h2 className='mt-2 text-lg text-gray-400'>{course?.completedExercises} Completed <span>out of {course?.totalExercises}</span></h2>
