...
import { Sparkles, Shield, Zap, Bot, Cpu, Target, Award, Users, ArrowRight } from "lucide-react";
import axios from "axios";
import { toast } from "sonner";
import { UserDetailContext } from "@/context/UserDetailContext";
import { upsertQuestLog } from "@/lib/quest-log";
...
export default function OnboardingPage() {
  ...
  const { setUserDetail } = useContext(UserDetailContext) as {
  ...
  const handleFinishOnboarding = async () => {
    setLoading(true);
    try {
      const result = await axios.post("/api/user/onboard", {
      ...
      const starter = result.data?.starterQuest;
      if (starter?.title && typeof starter.questId === "string") {
        upsertQuestLog(starter.questId, "IN_PROGRESS");
        toast.success(`Character saved. ${starter.title} is already in your quest log.`);
      } else {
        toast.success("Character saved. Welcome, adventurer!");
      }
      ...
      <div className="max-w-4xl w-full">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-game font-bold tracking-wide">
            ...
            <p className="text-gray-400 mt-2 font-game text-xl">
              Step {step} of 4: Guild Hall stays locked until you save a class and goal.
            </p>
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="flex justify-end pt-4">
              <Button
                variant="pixel"
                className="font-game text-2xl px-8"
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-neutral-900 border-neutral-800 p-6 space-y-6">
              ...
              <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 text-sm space-y-2">
                <div className="flex items-center gap-2 text-yellow-400 font-game text-lg">
                  <Award className="w-5 h-5" /> Starting Stats
                </div>
                ...
                <p className="text-xs text-gray-500">
                  Everyone starts at Level 1. Earn XP and gold by completing quests.
                </p>
              </div>
            </Card>
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button variant="pixel" className="font-game text-2xl px-8" onClick={() => setStep(4)}>
                Next: Confirm Character <ArrowRight className="ml-2 w-5 h-5" />
              </Button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-6 text-center">
            <h2 className="text-2xl font-game text-yellow-400">CONFIRM YOUR HERO</h2>
            <Card className="bg-neutral-900 border-yellow-500/30 p-8 space-y-6">
              <div className="w-20 h-20 bg-yellow-500/10 rounded-full border-2 border-yellow-400 flex items-center justify-center mx-auto text-yellow-400">
                <Users className="w-10 h-10" />
              </div>

              <div>
                <Badge variant="pixel" className="text-lg px-4 py-1">READY TO BEGIN</Badge>
                <h3 className="text-3xl font-game font-bold text-white mt-2">Level 1 Novice</h3>
                <p className="text-gray-400 text-sm max-w-md mx-auto mt-1">
                  Your hero starts empty. Accept quests, join a party, and earn gold as you ship.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-left font-mono text-xs">
                ...
                <div className="p-3 bg-neutral-950 rounded border border-neutral-800">
                  <div className="text-yellow-400 font-bold">Skill Level</div>
                  <div>{skillLevel}</div>
                </div>
              </div>
            </Card>
            ...
            <div className="flex justify-center pt-4">
              <Button
                ...
                disabled={loading || !selectedClass || !selectedGoal}
              >
                {loading ? "SAVING CHARACTER..." : "ENTER THE GUILD HALL 🚀"}
              </Button>
            </div>
          </div>
        )}
